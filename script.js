...
function processBonus() {
  const name=pick(DATA.bonuses); playSound('bonus'); state.bonusArtifactCount++; state.sessionCategories.bonus=true;
...
function encounterAngus(fromCompass=false) {
  playSound('angus'); state.angusEncounters++; if(fromCompass)state.angusFromCompass=true;
...
function achievements(finalFish,total) {
  ...
  if (giantFinal.length>=2) a.push('Мастер крупных форм');
  if (state.essenceUsed) a.push('Трансмутатор');
  if (state.angusGift) a.push('Дар великого мастера');

  // Рыба и вес
  if (state.goldenFishCaught) a.push('Золотая чешуя');
  if (state.heavyCaughtTotal>=3) a.push('Тяжёлая артиллерия');
  ...
  if (state.smallFishCaught>=5) a.push('Мелочь, а приятно');
  if (finalFish.length>=10) a.push('Рыбное изобилие');
  if (finalFish.length===1&&finalFish[0].weight>=20) a.push('Один, но могучий');

  // Погода
  if ((state.weatherSeen||[]).length>=4) a.push('Синоптик');
  ...
  if (state.weather==='storm'&&total>=50) a.push('Штормовой капитан');
  if (state.goldenHourFishCount>=3) a.push('Золотой улов');

  // Бонусы
  if (['Подводная маска','Ласты','Акваланг'].every(x=>uniqueBonusNames.has(x))) a.push('Полное снаряжение');
  if (state.blockedDebuffCount>0) a.push('Под защитой');
  if (state.luckyFloatSaves>0) a.push('Вторая попытка');
  if (state.flippersBoostedCount>=3) a.push('Ускоритель глубин');
  if (state.scubaAppliedTo15) a.push('Глубокое погружение');
  if (activeCoreGear) a.push('Морская машина');
  if (state.bonuses.filter(b=>b.name==='Акваланг').length>=2) a.push('Двойное погружение');
  if (uniqueBonusNames.size>=4) a.push('Арсенал рыбака');

  // Дебафы и восстановление
  if ((state.receivedDebuffNames||[]).length>=3&&total>=50) a.push('Переживший бурю');
  ...
  // Артефакты
  if (uniqueArtifactNames.size>=3) a.push('Коллекционер глубин');
  if (epicCount>=2) a.push('Эпическое путешествие');
  ...
  if (state.deepThingConvertedCount>0) a.push('Бездна ответила');
  if (state.nautilusActivatedWithTwoBonuses) a.push('Капитан Наутилуса');
  if (state.leviathanFishCount>=3) a.push('Дар Левиафана');
  ...
  if (state.trash.length>=5) a.push('Мусорный магнат');
  ...
  if (state.maxTrashStreak>=3) a.push('Не мой день');
...
const GUIDE = {
  ...
  'Достижения': [
    ...
    ['Любимчик Фортуны','Получить не менее 3 бонусов и/или артефактов.'],
    ['Неуловимый','Не получить ни одного дебафа; недоступно, если за сессию был Шторм.'],
    ...
    ['Гроза океана','Получить итоговый вес от 100 до 199,9 кг.'],
    ['Повелитель глубин','Получить итоговый вес от 200 кг.'],
    ['Благословение семи морей','Получить не менее 2 артефактов.'],
    ['Марафонец','Совершить не менее 10 забросов.'],
    ['Мастер крупных форм','Сохранить в итоговом улове не менее 2 гигантов.'],
    ['Трансмутатор','Успешно применить Эссенцию «Великан Океанов».'],
    ['Дар великого мастера','Получить артефакт от Ангуса.'],
    ['Золотая чешуя','Поймать золотую рыбку или золотую форель.'],
    ['Тяжёлая артиллерия','Поймать не менее 3 рыб-тяжеловесов за сессию.'],
    ...
    ['Мелочь, а приятно','Поймать не менее 5 рыб с исходным весом до 1 кг.'],
    ['Рыбное изобилие','Сохранить в итоговом улове не менее 10 рыб.'],
    ['Один, но могучий','Завершить игру с одной-единственной рыбой весом не менее 20 кг.'],
    ['Синоптик','Увидеть не менее 4 разных погодных режимов за сессию.'],
    ...
    ['Штормовой капитан','Завершить игру во время Шторма с весом не менее 50 кг.'],
    ['Золотой улов','Поймать не менее 3 рыб во время Золотого часа.'],
    ['Полное снаряжение','Получить Маску, Ласты и Акваланг за одну игру.'],
    ['Под защитой','Заблокировать хотя бы один дебаф Снаряжением дайвера.'],
    ['Вторая попытка','Сохранить заброс благодаря Счастливому поплавку.'],
    ['Ускоритель глубин','Усилить Ластами не менее 3 рыб.'],
    ...
    ['Двойное погружение','Получить не менее 2 Аквалангов.'],
    ['Арсенал рыбака','Получить не менее 4 разных бонусов.'],
    ['Переживший бурю','Получить не менее 3 разных дебафов и завершить игру с весом от 50 кг.'],
    ...
    ['Коллекционер глубин','Получить не менее 3 разных артефактов.'],
    ['Эпическое путешествие','Получить не менее 2 эпических артефактов.'],
    ...
    ['Бездна ответила','Превратить хлам в гиганта с помощью Глубоководного нечто.'],
    ['Капитан Наутилуса','Получить Штурвал при наличии не менее 2 активных бонусов.'],
    ['Дар Левиафана','Поймать не менее 3 рыб после получения Чешуи Левиафана.'],
    ...
    ['Мусорный магнат','Выловить не менее 5 единиц хлама.'],
    ...
    ['Не мой день','Получить хлам 3 раза подряд.'],
...
$('restartBtn').addEventListener('click',()=>{if(state.sessionDate){toast('Доступна только одна игровая сессия в сутки');return;}state=initialState();state.weatherSeen=[state.weather];if(state.weather==='storm')state.stormSeen=true;render();toast('Началась новая игровая сессия');});
